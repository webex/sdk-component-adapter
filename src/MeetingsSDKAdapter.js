...
export default class MeetingsSDKAdapter extends MeetingsAdapter {
  ...
  audioControl(ID) {
    ...
    return this.getMeeting(ID).pipe(
      map(({localAudio, disabledLocalAudio}) => (
        (localAudio && unmuted) || (disabledLocalAudio && muted) || disabled
      )),
      distinctUntilChanged(),
    );
  }
  ...
  videoControl(ID) {
    ...
    return this.getMeeting(ID).pipe(
      map(({localVideo, disabledLocalVideo}) => (
        (localVideo && unmuted) || (disabledLocalVideo && muted) || disabled
      )),
      distinctUntilChanged(),
    );
  }
